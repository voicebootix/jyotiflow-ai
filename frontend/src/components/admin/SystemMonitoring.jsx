...
import { 
  ...
  Database, Brain, Mic, Video, Share2, Search,
  BarChart3, Info, ChevronRight, ChevronDown
} from 'lucide-react';
...
const SystemMonitoring = () => {
  ...
  const fetchSessionDetails = async (sessionId) => {
    try {
      const response = await spiritualAPI.request(`/api/monitoring/session/${sessionId}`);
    ...
    setMonitoringData(prev => {
      ...
      switch (update.type) {
        ...
        case 'new_issue':
          return {
            ...prev,
            recent_issues: [update.data, ...(prev.recent_issues || [])].slice(0, 10)
          };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div className={`flex items-center space-x-1 px-3 py-1 rounded-full ${wsConnected ? 'bg-green-100' : 'bg-red-100'}`}>
              {wsConnected ? (
                <>
        ...
        <div className={`flex items-center justify-center p-4 rounded-lg ${
          monitoringData?.status === 'healthy' ? 'bg-green-50' :
          monitoringData?.status === 'degraded' ? 'bg-yellow-50' :
          'bg-red-50'
        }`}>
          ...
          <span className={`text-lg font-semibold ${
            monitoringData?.status === 'healthy' ? 'text-green-700' :
            monitoringData?.status === 'degraded' ? 'text-yellow-700' :
            'text-red-700'
          }`}>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        >
          <h3 className="text-lg font-semibold text-gray-800 flex items-center">
            {expandedSections.integrations ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
                  ...
                  <div className="text-sm text-gray-600 space-y-1">
                    {data.latency_ms && (
                      <div className="flex items-center justify-between">
                        <span>Latency:</span>
                        <span className="font-mono">{formatLatency(data.latency_ms)}</span>
                      </div>
                    )}
                    ...
                    {key === 'rag' && data.relevance_avg && (
                      <div className="flex items-center justify-between">
                        <span>Relevance:</span>
                        <span className={`font-mono ${data.relevance_avg < 65 ? 'text-red-600' : 'text-green-600'}`}>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        >
          <h3 className="text-lg font-semibold text-gray-800 flex items-center">
            {expandedSections.recentIssues ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs text-gray-500">{formatTimestamp(issue.timestamp)}</span>
                      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Success Rate */}
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Success Rate (24h)</h3>
          <div className="space-y-3">
            {Object.entries(monitoringData?.metrics?.success_rates || {}).map(([integration, rate]) => (
              <div key={integration} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-32 bg-gray-200 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full ${rate >= 95 ? 'bg-green-500' : rate >= 80 ? 'bg-yellow-500' : 'bg-red-500'}`}
                      style={{ width: `${rate}%` }}
                    />
                  </div>
                  <span className="text-sm font-mono w-12 text-right">{rate}%</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-3">
            {Object.entries(monitoringData?.metrics?.avg_response_times || {}).map(([integration, time]) => (
              <div key={integration} className="flex items-center justify-between">
                <span className="capitalize">{integration.replace('_', ' ')}</span>
                <span className={`font-mono ${time > 5000 ? 'text-red-600' : time > 2000 ? 'text-yellow-600' : 'text-green-600'}`}>
                  {formatLatency(time)}
                </span>
              </div>
            ))}
      ...
      {selectedSession && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[80vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold">Session Details: {selectedSession.session_id}</h3>
                <button 
              ...
              <div className="space-y-4">
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>Started: {formatTimestamp(selectedSession.started_at)}</div>
                    <div>Status: <span className={statusColors[selectedSession.status]}>{selectedSession.status}</span></div>
                    <div>Duration: {formatLatency(selectedSession.duration_ms)}</div>
                  </div>
                </div>
                    ...
                    {selectedSession.integrations?.map((integration, index) => (
                      <div key={index} className="flex items-center space-x-3 text-sm">
                        <span className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs ${
                          integration.status === 'success' ? 'bg-green-500' : 'bg-red-500'
                        }`}>
                          {index + 1}
                        </span>
                        <span className="font-medium">{integration.name}</span>
                        <span className="text-gray-500">{formatLatency(integration.duration_ms)}</span>
                        {integration.error && (
                          <span className="text-red-600 text-xs">{integration.error}</span>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-2 text-sm">
                      {selectedSession.validations.rag_relevance && (
                        <div>
                          RAG Relevance: <span className={selectedSession.validations.rag_relevance >= 65 ? 'text-green-600' : 'text-red-600'}>
                            {selectedSession.validations.rag_relevance}%
                          </span>
                        </div>
                      )}
                      {selectedSession.validations.context_preserved !== undefined && (
                        <div>
                          Context Preserved: <span className={selectedSession.validations.context_preserved ? 'text-green-600' : 'text-red-600'}>
                            {selectedSession.validations.context_preserved ? 'Yes' : 'No'}
                          </span>
                        </div>
                      )}
                    </div>
