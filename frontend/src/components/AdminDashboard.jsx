...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        ...
        const stats = await spiritualAPI.request(
          "/api/admin/analytics/overview"
        );
        if (stats) setAdminStats(stats);

        // Fetch credit packages for price management
        const packages = await spiritualAPI.request(
          "/api/admin/products/credit-packages"
        );
        if (packages)
          setCreditPackages(Array.isArray(packages) ? packages : []);
      } catch (error) {
  ...
  const handlePriceChange = async (packageId, newPrice) => {
    try {
      const result = await spiritualAPI.request(
        `/api/admin/products/credit-packages/${packageId}`,
        {
          method: "PUT",
      ...
      if (result && result.success) {
        ...
        setCreditPackages((prevPackages) =>
          prevPackages.map((pkg) =>
            pkg.id === packageId ? { ...pkg, price_usd: newPrice } : pkg
          )
        );
  ...
  const tabs = [
    ...
    { key: "subscriptionPlans", label: "💳 Subscription Plans" },
    ...
    { key: "testing", label: "🧪 Testing Dashboard" },
  ...
  return (
    <div className="pt-16 min-h-screen bg-gray-50">
        ...
        <div className="flex space-x-2 border-b border-gray-200 mb-8 overflow-x-auto">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2 font-medium rounded-t-lg focus:outline-none transition-colors duration-200 whitespace-nowrap ${
                activeTab === tab.key
                  ? "bg-white text-purple-800 border-x border-t border-gray-200 -mb-px"
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === "subscriptionPlans" && <SubscriptionPlans />}
        ...
        {activeTab === "testing" && <TestResultsDashboard />}
