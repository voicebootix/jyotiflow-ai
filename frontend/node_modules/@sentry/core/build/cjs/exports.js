...
function captureCheckIn(checkIn, upsertMonitorConfig) {
  ...
  if (!client) {
    debugBuild.DEBUG_BUILD && logger.debug.warn('Cannot capture check-in. No client defined.');
  } else if (!client.captureCheckIn) {
    debugBuild.DEBUG_BUILD && logger.debug.warn('Cannot capture check-in. Client does not support sending check-ins.');
  } else {
...
async function flush(timeout) {
  ...
  debugBuild.DEBUG_BUILD && logger.debug.warn('Cannot flush events. No client defined.');
...
async function close(timeout) {
  ...
  debugBuild.DEBUG_BUILD && logger.debug.warn('Cannot flush events and disable SDK. No client defined.');
  return Promise.resolve(false);
}
