...
class Client {
   ...
   */
   constructor(options) {
    ...
    if (options.dsn) {
      ...
      debugBuild.DEBUG_BUILD && logger.debug.warn('No DSN provided, client will not send events.');
    }
   ...
   */
   captureException(exception, hint, scope) {
    ...
    if (misc.checkOrSetAlreadyCaught(exception)) {
      debugBuild.DEBUG_BUILD && logger.debug.log(ALREADY_SEEN_ERROR);
      return eventId;
    }
   ...
   sendSession(session) {
    ...
    if ('aggregates' in session) {
      ...
      if (!sessionAttrs.release && !clientReleaseOption) {
        debugBuild.DEBUG_BUILD && logger.debug.warn(MISSING_RELEASE_FOR_SESSION_ERROR);
        return;
      }
   ...
   */
   recordDroppedEvent(reason, category, count = 1) {
    if (this._options.sendClientReports) {
      ...
      debugBuild.DEBUG_BUILD && logger.debug.log(`Recording outcome: "${key}"${count > 1 ? ` (${count} times)` : ''}`);
   ...
   sendEnvelope(envelope) {
    ...
    if (this._isEnabled() && this._transport) {
      return this._transport.send(envelope).then(null, reason => {
        debugBuild.DEBUG_BUILD && logger.debug.error('Error while sending envelope:', reason);
        return reason;
      });
    }

    debugBuild.DEBUG_BUILD && logger.debug.error('Transport disabled');
   ...
   */
   _captureEvent(
    ...
    if (debugBuild.DEBUG_BUILD && isErrorEvent(event)) {
      logger.debug.log(`Captured error event \`${eventUtils.getPossibleEventMessages(event)[0] || '<unknown>'}\``);
    }
      ...
      reason => {
        if (debugBuild.DEBUG_BUILD) {
          if (_isDoNotSendEventError(reason)) {
            logger.debug.log(reason.message);
          } else if (_isInternalError(reason)) {
            logger.debug.warn(reason.message);
          } else {
            logger.debug.warn(reason);
          }
        }
        return undefined;
      },
   ...
   */
   _flushOutcomes() {
    debugBuild.DEBUG_BUILD && logger.debug.log('Flushing outcomes...');

    const outcomes = this._clearOutcomes();

    if (outcomes.length === 0) {
      debugBuild.DEBUG_BUILD && logger.debug.log('No outcomes to send');
      return;
    }

    // This is really the only place where we want to check for a DSN and only send outcomes then
    if (!this._dsn) {
      debugBuild.DEBUG_BUILD && logger.debug.log('No dsn provided, will not send outcomes');
      return;
    }

    debugBuild.DEBUG_BUILD && logger.debug.log('Sending outcomes:', outcomes);
