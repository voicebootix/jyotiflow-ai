...
class SentrySpan  {
   ...
   */
   addEvent(
    ...
    debugBuild.DEBUG_BUILD && logger.debug.log('[Tracing] Adding an event to span:', name);
    ...
    if (this._isStandaloneSpan) {
      if (this._sampled) {
        ...
        debugBuild.DEBUG_BUILD &&
          logger.debug.log('[Tracing] Discarding standalone span because its trace was not chosen to be sampled.');
        if (client) {
   ...
   */
   _convertSpanToTransaction() {
    ...
    if (!this._name) {
      debugBuild.DEBUG_BUILD && logger.debug.warn('Transaction has no name, falling back to `<unlabeled transaction>`.');
