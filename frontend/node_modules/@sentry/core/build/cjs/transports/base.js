      ...
      makeRequest({ body: envelope.serializeEnvelope(filteredEnvelope) }).then(
        response => {
          ...
          if (response.statusCode !== undefined && (response.statusCode < 200 || response.statusCode >= 300)) {
            debugBuild.DEBUG_BUILD && logger.debug.warn(`Sentry responded with status code ${response.statusCode} to sent event.`);
          }
        ...
        error => {
          ...
          debugBuild.DEBUG_BUILD && logger.debug.error('Encountered error running transport request:', error);
          throw error;
        },
      );
    ...
    return buffer.add(requestTask).then(
      ...
      error => {
        if (error === promisebuffer.SENTRY_BUFFER_FULL_ERROR) {
          debugBuild.DEBUG_BUILD && logger.debug.error('Skipped sending event because buffer is full.');
