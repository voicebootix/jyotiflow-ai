...
('The Path to Inner Peace', 'True inner peace is not the absence of conflict, but the presence of tranquility amidst life''s storms. When we cultivate stillness within, we become unshakeable by external circumstances. Practice daily meditation, even for just 10 minutes, and watch as your inner landscape transforms. Peace is your natural state - everything else is temporary noise.', 'meditation', ARRAY['inner_peace', 'meditation', 'tranquility', 'mindfulness'], 'https://jyotiflow.com/inner-peace', '{"difficulty": "beginner", "duration": "daily", "practice_type": "meditation"}'),
...
-- Update metadata for all entries to include creation timestamp
UPDATE rag_knowledge_base SET 
    metadata = jsonb_set(
        COALESCE(metadata, '{}'::jsonb), 
        '{created_for}', 
        '"spiritual_calendar_service"'::jsonb
    ),
