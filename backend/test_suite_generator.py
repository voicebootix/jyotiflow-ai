...
async def test_admin_endpoint_generic(endpoint_index, test_name, test_type):
    ...
    try:
        ...
        conn = await asyncpg.connect(database_url)
        async with conn.transaction():
            endpoints = await conn.fetch("SELECT endpoint, method, business_function, test_data FROM admin_endpoints_config ORDER BY id LIMIT 4")
            if len(endpoints) != 4:
                return {"status": "failed", "error": f"Expected 4 admin endpoints, found {len(endpoints)}"}
            required_keys = ['endpoint', 'method', 'business_function']
            for idx, endpoint_row in enumerate(endpoints):
                missing_keys = [key for key in required_keys if key not in endpoint_row or endpoint_row.get(key) is None]
                if 'test_data' not in endpoint_row:
                    endpoint_row = dict(endpoint_row)
                    endpoint_row['test_data'] = {}
                    endpoints[idx] = endpoint_row
                if missing_keys:
                    return {"status": "failed", "error": f"Malformed endpoint configuration at index {idx}", "details": {"missing_keys": missing_keys, "endpoint_index": idx, "available_keys": list(endpoint_row.keys()) if endpoint_row else [], "validation_type": "structure_validation"}}
                if not endpoint_row.get('endpoint') or not isinstance(endpoint_row.get('endpoint'), str):
                    return {"status": "failed", "error": f"Invalid endpoint value at index {idx}", "details": {"endpoint_index": idx, "endpoint_value": endpoint_row.get('endpoint'), "validation_type": "value_validation"}}
                if not endpoint_row.get('method') or endpoint_row.get('method').upper() not in ['GET', 'POST', 'PUT', 'DELETE', 'PATCH']:
                    return {"status": "failed", "error": f"Invalid HTTP method at index {idx}", "details": {"endpoint_index": idx, "method_value": endpoint_row.get('method'), "allowed_methods": ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'], "validation_type": "method_validation"}}
                if not endpoint_row.get('business_function') or not isinstance(endpoint_row.get('business_function'), str):
                    return {"status": "failed", "error": f"Invalid business_function value at index {idx}", "details": {"endpoint_index": idx, "business_function_value": endpoint_row.get('business_function'), "validation_type": "business_function_validation"}}
            if endpoint_index >= len(endpoints):
                return {"status": "failed", "error": f"Endpoint index {endpoint_index} out of range"}
            endpoint_config = endpoints[endpoint_index]
            session_id = str(uuid.uuid4())
            await conn.execute("INSERT INTO test_execution_sessions (session_id, test_type, test_category, status, created_at) VALUES ($1, $2, $3, $4, NOW())", session_id, test_type, 'admin_services_critical', 'running')
            test_config_row = await conn.fetchrow("SELECT value FROM platform_settings WHERE key = 'admin_test_config'")
            if test_config_row and test_config_row['value']:
                test_config = json.loads(test_config_row['value']) if isinstance(test_config_row['value'], str) else test_config_row['value']
            else:
                test_config = {"api_base_url": "https://jyotiflow-ai.onrender.com", "expected_codes": [200, 401, 403]}
            url = f"{test_config['api_base_url']}{endpoint_config['endpoint']}"
            method = endpoint_config['method']
            test_data = endpoint_config['test_data']
            business_function = endpoint_config['business_function']
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    status_code = response.status_code
                    accessible = status_code in test_config['expected_codes']
                    await conn.execute("INSERT INTO monitoring_api_calls (endpoint, method, status_code, response_time, timestamp) VALUES ($1, $2, $3, $4, NOW())", endpoint_config['endpoint'], method, status_code, response_time_ms)
                    test_status = 'passed' if accessible else 'failed'
                    await conn.execute("INSERT INTO test_case_results (session_id, test_name, test_category, status, test_data, output_data, created_at) VALUES ($1, $2, $3, $4, $5, $6, NOW())", session_id, test_name, 'admin_services_critical', test_status, json.dumps(test_data), json.dumps({"status_code": status_code, "response_time_ms": response_time_ms}))
                    await conn.execute("UPDATE test_execution_sessions SET status = 'completed', finished_at = NOW() WHERE session_id = $1", session_id)
                    ...
                    if result and result.get('output_data'):
                        try:
                            ...
                            print(f"JSON parse error in session {session_id}: {json_error}")
                            details = {"error": "JSON parse failed", "raw_output_available": True}
                    return {"status": result['status'] if result else "failed", "business_function": business_function, "details": details}
            except Exception as http_error:
                if session_id and conn:
                    try:
