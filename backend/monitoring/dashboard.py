...
# Add test endpoints without authentication for dashboard testing
@router.get("/test-sessions-public")
async def get_test_sessions_public():
        ...
        try:
            ...
            sessions = await conn.fetch("""
                SELECT session_id, status, start_time, end_time, 
                       ...
                       execution_environment, test_suite
                FROM test_execution_sessions 
...
@router.get("/test-metrics-public")
async def get_test_metrics_public():
    """Get test execution metrics and statistics - public endpoint for testing"""
    try:
        conn = await db_manager.get_connection()
        try:
            ...
            # Get basic metrics
            total_sessions = await conn.fetchval("SELECT COUNT(*) FROM test_execution_sessions") or 0
            
            passed_sessions = await conn.fetchval("""
                SELECT COUNT(*) FROM test_execution_sessions WHERE status = 'completed'
            """) or 0
            ...
            return {
                ...
                "data": {
                    ...
                    "coverage_trend": 85,  # Mock data
                    "auto_fixes_applied": 12  # Mock data
                }
            }
