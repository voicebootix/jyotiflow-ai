    ...
    async def _get_test_cases(self, suite_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                # ✅ FOLLOWING .CURSOR RULES: Direct database query to test_case_results
                # Look directly in test_case_results for matching test_category
                # This uses your actual data without complex JOINs
                results = await conn.fetch('''
                    SELECT DISTINCT ON (test_name)
                        test_name, test_category, test_data, output_data, status
            ...
            for row in results:
                try:
                    ...
                    logger.info(f"Loaded test case '{row['test_name']}' with status '{row['status']}' from test_case_results table")
                except Exception as e:
