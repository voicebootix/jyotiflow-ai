    ...
    async def _get_test_cases(self, suite_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                # ✅ FOLLOWING .CURSOR RULES: Database-driven status filtering, no hardcoded values
                # Get all test cases for the suite, regardless of status - let the execution engine decide
                results = await conn.fetch('''
                    SELECT tcr.test_name, tcr.test_category, tcr.test_data, tcr.output_data, tcr.status
