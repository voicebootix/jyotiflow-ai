    ...
    async def _get_test_cases(self, suite_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                # ✅ FOLLOWING .CURSOR RULES: Database-driven deduplication and scoping
                # Get latest test case per test_name from generator sessions only
                results = await conn.fetch('''
                    SELECT DISTINCT ON (tcr.test_name)
                        tcr.test_name, tcr.test_category, tcr.test_data, tcr.output_data, tcr.status
                    FROM test_case_results tcr
                    JOIN test_execution_sessions tes ON tcr.session_id = tes.session_id
                    WHERE tes.test_type = $1
                      AND tes.triggered_by = 'test_suite_generator'
                    ORDER BY tcr.test_name, tcr.created_at DESC
                ''', suite_name)
            ...
            for row in results:
                try:
                    # ✅ FOLLOWING .CURSOR RULES: Database-driven data parsing, include status from database
                    ...
                    test_cases.append({
                        ...
                        'database_status': row['status'],  # Include actual database status
                        **test_data
                    })
                    logger.info(f"Loaded test case '{row['test_name']}' with status '{row['status']}' from database")
                except Exception as e:
