...
async def get_platform_config(
    ...
    try:
        ...
        config_data = {}
        platform_models = {
            'youtube': YouTubePlatformStatus, 'facebook': FacebookPlatformStatus,
            'instagram': InstagramPlatformStatus, 'tiktok': TikTokPlatformStatus
        }
        for row in rows:
            key = row['key']
...
async def upload_swamiji_image(
    ...
    try:
        public_url = storage_service.upload_file(
            bucket_name="avatars", file_path_in_bucket=file_name_in_bucket,
...
async def generate_all_avatar_previews(
    ...
    try:
        ...
        if not themed_image_url:
            raise HTTPException(status_code=500, detail="Failed to get themed image URL.")
        result = await avatar_engine.generate_avatar_preview_lightweight(
            guidance_text=request.sample_text,
            voice_id=request.voice_id,
...
async def approve_swamiji_avatar(
    ...
    try:
        async with conn.transaction():
            config_value = {
                "approved_at": datetime.now().isoformat(), "approved_by": admin_user.get("email"),
                "base_image_url": request.image_url, "approved_video_url": request.video_url,
                "generation_prompt": request.prompt,
            }
            await conn.execute(
                "INSERT INTO platform_settings (key, value, updated_at) VALUES ('swamiji_avatar_url', $1, NOW()) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()",
                json.dumps(request.image_url)
            )
            await conn.execute(
                "INSERT INTO platform_settings (key, value, updated_at) VALUES ('swamiji_approved_config', $1, NOW()) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()",
                json.dumps(config_value)
            )
        return StandardResponse(success=True, message="Avatar configuration approved.", data={"configuration": config_value})
    except Exception as e:
        logger.error(f"Failed to save approved Swamiji avatar config: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to save approved configuration.") from e
...
async def execute_posting(request: PostExecutionRequest, admin_user: dict = Depends(AuthenticationHelper.verify_admin_access_strict)):
    result = PostExecutionResult(
        success=True, message="Content posting scheduled.", task_id="task_12345",
...
async def create_marketing_asset(asset: MarketingAssetCreate, admin_user: dict = Depends(AuthenticationHelper.verify_admin_access_strict)):
    new_asset = MarketingAsset(id=1, name=asset.name, type=asset.type, url=asset.url, created_at="2024-08-15T15:00:00Z")
